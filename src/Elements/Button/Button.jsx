...
export default function Button(props) {
    const { mt="mt-4" , link , shadow="shadow-[0_4px_20px_rgba(255,255,255,0.5)]" , bg="bg-ccream" , textGoo="text-cmaroon" , text="Detail" , bgGo="bg-cmaroon" , textNext="text-ccream" } = props;
    return(
        <div className={`${mt}`}>
            <Link to={link}>
                <div className={`${shadow} ${bg} w-36 pl-5 pr-2 py-2 rounded-lg flex justify-between text-center items-center`}>
                    <div className={`${textGoo}`}>{text}</div>
